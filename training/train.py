...
import training.modules.commons as commons
from training.data_utils import TextAudioCollate, TextAudioSpeakerLoader
from training.models import (
    MultiPeriodDiscriminator,
    SynthesizerTrn,
)
from training.modules.losses import discriminator_loss, feature_loss, generator_loss, kl_loss
from training.modules.mel_processing import mel_spectrogram_torch, spec_to_mel_torch
...
def run(rank, n_gpus, hps):
    ...
    try:
        _, _, _, epoch_str = load_checkpoint(
            latest_checkpoint_path(hps.model_dir, "G_*.pth"), net_g, optim_g, skip_optimizer
        )
        _, _, _, epoch_str = load_checkpoint(
            latest_checkpoint_path(hps.model_dir, "D_*.pth"), net_d, optim_d, skip_optimizer
        )
        epoch_str = max(epoch_str, 1)
        name = latest_checkpoint_path(hps.model_dir, "D_*.pth")
    ...
    for batch_idx, items in enumerate(train_loader):
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                ...
                if keep_ckpts > 0:
                    clean_checkpoints(path_to_models=hps.model_dir, n_ckpts_to_keep=keep_ckpts, sort_by_time=True)
