...
from training.modules.F0Predictor.CrepeF0Predictor import CrepeF0Predictor
from training.modules.F0Predictor.DioF0Predictor import DioF0Predictor
from training.modules.F0Predictor.F0Predictor import F0Predictor
from training.modules.F0Predictor.FCPEF0Predictor import FCPEF0Predictor
from training.modules.F0Predictor.HarvestF0Predictor import HarvestF0Predictor
from training.modules.F0Predictor.PMF0Predictor import PMF0Predictor
from training.modules.F0Predictor.RMVPEF0Predictor import RMVPEF0Predictor
from training.vencoder.CNHubertLarge import CNHubertLarge
from training.vencoder.ContentVec256L9 import ContentVec256L9
from training.vencoder.ContentVec768L12 import ContentVec768L12
from training.vencoder.DPHubert import DPHubert
from training.vencoder.encoder import SpeechEncoder
from training.vencoder.HubertSoft import HubertSoft
from training.vencoder.WavLMBasePlus import WavLMBasePlus
from training.vencoder.WhisperPPG import WhisperPPG
from training.vencoder.WhisperPPGLarge import WhisperPPGLarge
...
def plot_data_to_numpy(x: npt.NDArray[np.float32], y: npt.NDArray[np.float32]) -> npt.NDArray[np.uintc]:
...
def get_content(cmodel: torch.nn.Module, y: torch.Tensor) -> Any:
    with torch.no_grad():
...
def get_f0_predictor(f0_predictor: str, hop_length: int, sampling_rate: int, **kargs: Any) -> F0Predictor:
    f0_predictor_object: F0Predictor
    if f0_predictor == "pm":
...
def get_speech_encoder(speech_encoder: str, device: Optional[str] = None, **kargs: Any) -> SpeechEncoder:
    speech_encoder_object: SpeechEncoder
...
def load_checkpoint(
    checkpoint_path: str, model: Any, optimizer: Optional[Any] = None, skip_optimizer: bool = False
    ...
    print("load ")
    logging.info("Loaded checkpoint '{}' (iteration {})".format(checkpoint_path, iteration))
...
def clean_checkpoints(path_to_models: str = "logs/44k/", n_ckpts_to_keep: int = 2, sort_by_time: bool = True) -> None:
    ...
    def name_key(_f: str) -> int:
        reg_exp = re.compile("._(\\d+)\\.pth").match(_f)
        if reg_exp is not None:
            return int(reg_exp.group(1))
        else:
            raise ValueError("Return None in pattern matching code...")
    ...
    def del_info(fn: str) -> None:
        logging.info(f".. Free up space by deleting ckpt {fn}")

    def del_routine(x: str) -> None:
        os.remove(x)
        del_info(x)
...
class InferHParams(HParams):
    ...
    def __getattr__(self, index: Any) -> Any:
...
def plot_spectrogram_to_numpy(spectrogram: torch.Tensor) -> npt.NDArray[np.uint8]:
    global MATPLOTLIB_FLAG
    if not MATPLOTLIB_FLAG:
        matplotlib.use("Agg")
        MATPLOTLIB_FLAG = True
...
def plot_alignment_to_numpy(alignment: torch.Tensor, info: Optional[str] = None) -> npt.NDArray[np.uint8]:
    global MATPLOTLIB_FLAG
    if not MATPLOTLIB_FLAG:
        matplotlib.use("Agg")
        MATPLOTLIB_FLAG = True
    ...
    im = ax.imshow(alignment.transpose(0, 1), aspect="auto", origin="lower", interpolation="none")
...
def get_hparams(init: bool = True) -> HParams:
    ...
    hparams.model_dir = model_dir  # type: ignore
    return hparams
...
def get_hparams_from_file(config_path: str, infer_mode: bool = False) -> Any:
    with open(config_path, "r") as f:
        data = f.read()
    config = json.loads(data)
...
def check_git_hash(model_dir: str) -> None:
    ...
    if not os.path.exists(os.path.join(source_dir, ".git")):
        logging.warn("{} is not a git repository, therefore hash value comparison will be ignored.".format(source_dir))
        return
    ...
    if os.path.exists(path):
        ...
        if saved_hash != cur_hash:
            logging.warn("git hash values are different. {}(saved) != {}(current)".format(saved_hash[:8], cur_hash[:8]))
    else:
        open(path, "w").write(cur_hash)


def get_logger(model_dir: str, filename: str = "train.log") -> Any:
    # global logger
...
def repeat_expand_2d(content: torch.Tensor, target_len: int, mode: str = "left") -> Any:
    # content : [h, t]
    return (
...
def repeat_expand_2d_other(content: torch.Tensor, target_len: int, mode: str = "nearest") -> Any:
    # content : [h, t]
...
def mix_model(model_paths: List[str], mix_rate: torch.Tensor, mode: int) -> str:
...
def change_rms(data1: npt.NDArray[np.float32], sr1: int, data2: torch.Tensor, sr2: int, rate: float) -> torch.Tensor:
    # print(data1.max(),data2.max())
    rms1 = torch.from_numpy(librosa.feature.rms(y=data1, frame_length=sr1 // 2 * 2, hop_length=sr1 // 2)).to(
        data2.device
    )
    rms2 = torch.from_numpy(
        librosa.feature.rms(y=data2.detach().cpu().numpy(), frame_length=sr2 // 2 * 2, hop_length=sr2 // 2)
    ).to(data2.device)
...
) -> Any:  # from: RVC https://github.com/RVC-Project/Retrieval-based-Voice-Conversion-WebUI
    n_cpu = cpu_count()
...
class Volume_Extractor:
    def __init__(self, hop_size: int = 512) -> None:
        self.hop_size = hop_size

    def extract(self, audio: torch.Tensor) -> torch.Tensor:  # audio: 2d tensor array
