...
from training.modules.F0Predictor.rmvpe.constants import N_MELS
...
class ResEncoderBlock(nn.Module):
    def __init__(
        self, in_channels: int, out_channels: int, kernel_size: Optional[int], n_blocks: int = 1, momentum: float = 0.01
    ) -> None:
...
class Decoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, concat_tensors: torch.Tensor) -> torch.Tensor:
        for i in range(self.n_decoders):
...
class TimbreFilter(nn.Module):
    def __init__(self, latent_rep_channels: List[List[int]]) -> None:
    ...
    def forward(self, x_tensors: torch.Tensor) -> torch.Tensor:
        out_tensors = []
